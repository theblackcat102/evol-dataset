...
def format_for_chat_completion(prompt):
    if isinstance(prompt, str):
        ...
        messages = [ {"role": "user" if idx % 2 == 0 else "assistant", "content": p }  for idx, p in enumerate(prompt) ]
...
class OpenAICompletion():
    ...
    def completion(self, *args, **kwargs):
        if self.azure_backend_enabled:
            return self.azure_completion(*args, **kwargs)
        return self.openai_completion(*args, **kwargs)

    def azure_completion(self, prompt,
        model_name="gpt-3.5-turbo",
        temperature=0.7,
        max_tokens=2048, top_p=0, 
        frequency_penalty=0, 
        ...
        while not success:
            try:
                ...
                url = openai.api_base + "/openai/deployments/" + deployment_name + "/chat/completions"
                payload = {
                    "messages": format_for_chat_completion(prompt),
                    "stop": stop,
                    "temperature": temperature,
                ...
                elapsed = time() - start
                if 'choices' in response and len(response['choices']) and \
                ...
                err = str(e)
                if "Azure OpenAI’s content management policy" in err or \
                    'The server is currently overloaded with other requests' in err or \
                    'custom content moderation triggered' in err:
                    # fallback to openai method
                    payload = self.openai_completion(model_name=model_name,
                            prompt=prompt,
                            temperature=temperature,
                            max_tokens=max_tokens,
                            top_p=top_p,
